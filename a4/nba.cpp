    ...
    while (!re.empty()){
        ...
        if (parametersVec[0] == "lw")
        {
            ...
            if (rowbuff.row_no == -1){
                ...
                rowbuff.UpdateRowBuffer(add / 1024, add %1024, Rdest, curr );
                ...
                //COLUMN
                print_req();
                rowbuff.UpdateRowBuffer(rowbuff.row_no, add %1024, Rdest, -1 );
            ...
            else
            {
                ...
                if (add / 1024 != rowbuff.row_no){
                    print_req();
                        if (rowbuff.row_no == -1){
                            //cout << "Hello7"<< endl;
                            rowbuff.UpdateRowBuffer(add / 1024, add %1024, reg, -1 );
                            rowbuff.UpdateDelays(row_access_delay, col_access_delay);
                        }
                        else{
                            //cout << "Hello8"<< endl;
                            int curr = rowbuff.row_no;
                            rowbuff.UpdateRowBuffer(add / 1024, add %1024, reg, curr );
                            rowbuff.UpdateDelays(2 * row_access_delay, col_access_delay);
                        }
                    }
                    else{
                        //COLUMN
                        print_req();
                        rowbuff.UpdateRowBuffer(rowbuff.row_no, add %1024, reg, -1);
                        rowbuff.UpdateDelays(0, col_access_delay);
                    }
            }
